  ...
  async function navigateToDetail(sponsoredName){
   ...
   async function cancelSponsored(sponsoredName) {
    ...
    const res = await fetch(`${API_BASE_URL}/coupons/cancel`, {
      method: "POST",
      headers: getAuthHeader(),
      body: JSON.stringify({ sponsoredName: sponsoredName })
    })
    const data = await res.json()
              ...
              <tbody>
                {sponsoredSummary.map((shop,index) => (
                  <tr key={index}>
                    ...
                    <td><button onClick={() => navigateToDetail(shop._id)}>Detail</button> <button onClick={() => cancelSponsored(shop._id)}>Cancel</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
